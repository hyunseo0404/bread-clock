...
const (
	...
	SortByDistance            = "coordinates" // FIXME
)

type BakeryRepository interface {
	List(ctx context.Context, sortOption SortOption, size int, offset int, userID int) ([]models.BakeryDetail, error)
	ListForBreads(ctx context.Context, q string, sortOption SortOption, size int, offset int, userID int) ([]models.BakeryDetail, error)
	Get(ctx context.Context, bakeryID int, userID int) (*models.BakeryDetail, error)
...
func (r *bakeryRepository) List(ctx context.Context, sortOption SortOption, size int, offset int, userID int) ([]models.BakeryDetail, error) {
	...
	query := tx.
		Select("b.id, b.name, b.address, b.coordinates, b.opening_hours, b.user_id, b.bakery_id, GROUP_CONCAT(url) as photo_urls").
		Table("(?) AS b", subQuery).
		Joins("JOIN bakery_photos AS bp ON bp.bakery_id = b.id").
		Group("b.id")
	...
	for _, bakeryDAO := range bakeryDAOs {
		bakeries = append(bakeries, models.BakeryDetail{
			Bakery: models.Bakery{
				...
				Coordinates:  bakeryDAO.Coordinates,
...
func (r *bakeryRepository) ListForBreads(ctx context.Context, q string, sortOption SortOption, size int, offset int, userID int) ([]models.BakeryDetail, error) {
	tx := r.db.WithContext(ctx)
	...
	subQuery = tx.
		...
		Limit(size)
	query = tx.
		Select("b.id, b.name, b.address, b.coordinates, b.opening_hours, user_id, GROUP_CONCAT(url) AS photo_urls").
		Table("(?) AS b", subQuery).
		Joins(fmt.Sprintf("LEFT JOIN favorite_bakeries AS fb ON fb.bakery_id = b.id AND user_id = %d", userID)).
		Joins("JOIN bakery_photos AS bp ON bp.bakery_id = b.id").
		Order(string(sortOption)).
		Group("b.id")
...
func (r *bakeryRepository) Get(ctx context.Context, bakeryID int, userID int) (*models.BakeryDetail, error) {
	tx := r.db.WithContext(ctx)

	var bakeryDAO BakeryDAO
	err := tx.Table("bakeries AS b").
		Select("b.id, b.name, b.address, b.coordinates, b.opening_hours, fb.user_id, GROUP_CONCAT(url) AS photo_urls").
		Joins(fmt.Sprintf("LEFT JOIN favorite_bakeries AS fb ON b.id = fb.bakery_id AND fb.user_id = %d", userID)).
